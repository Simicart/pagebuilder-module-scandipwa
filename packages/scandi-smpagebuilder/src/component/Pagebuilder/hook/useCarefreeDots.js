...
/** @namespace ScandiSmpagebuilder/Component/Pagebuilder/Hook/UseCarefreeDots/useCarefreeDots */
export const useCarefreeDots = (props) => {
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            // note: change this query if parent class changes
            ...
            if (childContainerEl) {
                ...
                for (let indx = elements.length - 1; indx >= 0; indx--) {
                    ...
                    const { marginRight } = getComputedStyle(target);
                    childByPosRef.current[elements.length - (1 + indx)] = widthFromEnd;

                    widthFromEnd += parseInt(marginRight, 10) + target.offsetWidth;
                ...
                if (itemToMinus < numberOfChildren) {
                    ...
                    setNumberOfSteps(numberOfChildren);
                }
            }
        }, 1000);
